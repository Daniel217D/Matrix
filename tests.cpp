...
TEST_CASE("Matrix::inverse", "[Matrix]") {
    SECTION("input_0.txt") {
        ...
        for (int i = 0; i < inverse.rows && noError; ++i) {
            for (int j = 0; j < inverse.cols && noError; ++j) {
                if (std::abs(inverse.matrix[i][j] - expectation[i][j]) > EPS) {
                    noError = false;
                }
            }
        }
    ...
    SECTION("input_hard.txt") {
        ...
        REQUIRE(inverse.cols == 5);
        REQUIRE(inverse.rows == 5);
